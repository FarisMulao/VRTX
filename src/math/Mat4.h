...
struct Mat4 {
  ...
  Mat4() {
    DirectX::XMStoreFloat4x4((DirectX::XMFLOAT4X4 *)m,
  ...
  Mat4 operator*(const Mat4 &o) const {
    DirectX::XMMATRIX m1 =
        DirectX::XMLoadFloat4x4((const DirectX::XMFLOAT4X4 *)m);
    DirectX::XMMATRIX m2 =
        DirectX::XMLoadFloat4x4((const DirectX::XMFLOAT4X4 *)o.m);
  ...
  static Mat4 LookAtLH(const Vec3 &eye, const Vec3 &focus, const Vec3 &up) {
    DirectX::XMVECTOR vEye =
        DirectX::XMVectorSet(eye.x_val, eye.y_val, eye.z_val, 1.0f);
    DirectX::XMVECTOR vFocus =
        DirectX::XMVectorSet(focus.x_val, focus.y_val, focus.z_val, 1.0f);
    DirectX::XMVECTOR vUp =
        DirectX::XMVectorSet(up.x_val, up.y_val, up.z_val, 0.0f);
