...
void D3D12Context::InitD3D() {
    ...
    for (UINT i = 0; DXGI_ERROR_NOT_FOUND != m_dxgiFactory->EnumAdapters1(i, &adapter); ++i) {
...
void D3D12Context::CreateRtvAndDsvDescriptorHeaps() {
    ...
    m_device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &depthDesc, D3D12_RESOURCE_STATE_DEPTH_WRITE, &optClear, IID_PPV_ARGS(&m_depthStencilBuffer));
...
void D3D12Context::Resize(int width, int height) {
    ...
    for (int i = 0; i < FrameCount; ++i) m_swapChainBuffers[i].Reset();
