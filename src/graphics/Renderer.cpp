...
static HRESULT CompileShader(const char *filename, const char *entry,
                             const char *target, ID3DBlob **blob,
                             ID3DBlob **errorBlob) {
  std::wstring absPath =
      ToWide(std::string(VRTX_SOURCE_DIR) + "/src/shaders/" + filename);
  std::wstring relPath = ToWide(std::string(filename));

  HRESULT hr = D3DCompileFromFile(absPath.c_str(), nullptr, nullptr, entry,
                                  target, D3DCOMPILE_DEBUG, 0, blob, errorBlob);
  if (FAILED(hr))
    hr = D3DCompileFromFile(relPath.c_str(), nullptr, nullptr, entry, target,
                            D3DCOMPILE_DEBUG, 0, blob, errorBlob);
  return hr;
}

Renderer::Renderer(std::shared_ptr<D3D12Context> context)
    : m_context(context), m_indexCount(0), m_maxInstances(10000),
      m_currentInstanceCount(0), m_containerPointCount(0), m_bondVertexCount(0),
      m_maxBondVertices(20000) {}
...
void Renderer::BuildRootSignature() {
  ...
  D3D12SerializeRootSignature(&rootSigDesc, D3D_ROOT_SIGNATURE_VERSION_1,
                              &signature, &error);
  m_context->GetDevice()->CreateRootSignature(0, signature->GetBufferPointer(),
...
void Renderer::BuildPSO() {
  ...
  HRESULT hr = CompileShader("InstancedRenderer.hlsl", "VSMain", "vs_5_0",
                             &vsBlob, &errorBlob);
  if (FAILED(hr)) {
    OutputDebugStringA(
        "[VRTX] FATAL: Failed to compile InstancedRenderer.hlsl (VSMain)\n");
    if (errorBlob)
  ...
  errorBlob.Reset();
  hr = CompileShader("InstancedRenderer.hlsl", "PSMain", "ps_5_0", &psBlob,
                     &errorBlob);
  if (FAILED(hr)) {
    OutputDebugStringA(
        "[VRTX] FATAL: Failed to compile InstancedRenderer.hlsl (PSMain)\n");
    if (errorBlob)
      OutputDebugStringA((char *)errorBlob->GetBufferPointer());
    return;
  }
  ...
  OutputDebugStringA(
      "[VRTX] Shader InstancedRenderer.hlsl compiled successfully!\n");
  ...
  m_context->GetDevice()->CreateGraphicsPipelineState(
...
void Renderer::BuildContainerPSO() {
  ...
  HRESULT hr = CompileShader("ContainerShader.hlsl", "VSMain", "vs_5_0",
                             &vsBlob, &errorBlob);
  if (FAILED(hr)) {
    OutputDebugStringA(
        "[VRTX] WARN: Failed to compile ContainerShader.hlsl (VSMain)\n");
    if (errorBlob)
      OutputDebugStringA((char *)errorBlob->GetBufferPointer());
    return;
  }
  errorBlob.Reset();
  hr = CompileShader("ContainerShader.hlsl", "PSMain", "ps_5_0", &psBlob,
                     &errorBlob);
  if (FAILED(hr)) {
    OutputDebugStringA(
        "[VRTX] WARN: Failed to compile ContainerShader.hlsl (PSMain)\n");
    if (errorBlob)
      OutputDebugStringA((char *)errorBlob->GetBufferPointer());
    return;
  }
  ...
  OutputDebugStringA(
      "[VRTX] Shader ContainerShader.hlsl compiled successfully!\n");
  ...
  m_context->GetDevice()->CreateGraphicsPipelineState(
      &psoDesc, IID_PPV_ARGS(&m_containerPSO));
}

static void UploadContainerPoints(ID3D12Device *device,
...
void Renderer::RebuildContainerSphere(float radius, int latDiv, int lonDiv) {
  ...
  m_context->Flush();
  UploadContainerPoints(m_context->GetDevice(), points, m_containerVB,
                        m_containerVBView, m_containerPointCount);
...
void Renderer::BuildBondPSO() {
  ...
  m_context->GetDevice()->CreateGraphicsPipelineState(&psoDesc,
                                                      IID_PPV_ARGS(&m_bondPSO));
}
...
void Renderer::UpdateBonds(const std::vector<Constraint> &constraints,
                           const std::vector<VerletBody> &bodies) {
  ...
  if (m_bondVertexCount == 0)
    return;
  if (m_bondVertexCount > m_maxBondVertices)
    m_bondVertexCount = m_maxBondVertices;
...
void Renderer::BuildGeometry() {
  Mesh sphere = Mesh::CreateSphere(8, 8);
...
void Renderer::UpdateInstances(const std::vector<VerletBody> &bodies) {
  ...
  m_currentInstanceCount =
      (uint32_t)(bodies.size() < m_maxInstances ? bodies.size()
                                                : m_maxInstances);

  void *mappedData;
  m_instanceBuffer->Map(0, nullptr, &mappedData);
  memcpy(mappedData, bodies.data(),
         m_currentInstanceCount * sizeof(VerletBody));
...
void Renderer::Render(const Camera &camera) {
  ...
  if (m_pipelineState && m_currentInstanceCount > 0) {
    ...
    cmdList->DrawIndexedInstanced(m_indexCount, m_currentInstanceCount, 0, 0,
                                  0);
  }
}
