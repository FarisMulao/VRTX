...
class SpatialGrid {
public:
    SpatialGrid(float cellSize = 1.0f) : m_cellSize(cellSize), m_invCellSize(1.0f / cellSize) {}
...
private:
    ...
    int PosToCell(float v) const {
        return (int)std::floor(v * m_invCellSize);
    }
    ...
    int64_t HashPos(Vec3 pos) const {
        return CellHash(PosToCell(pos.x_val), PosToCell(pos.y_val), PosToCell(pos.z_val));
    }
};
